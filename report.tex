...
\begin{itemize}
    ...
    Pegando a mensagem $M$ e calculando o seu resumo $d = \mathcal{H}(M)$. Por conveniência, nós escolhemos $m$ ou $w$ para que $w \mid m$. However, one may pad the digest by appending zeros to satisfy this requirement. $d$ is then split into a $t_1$-tuple of base-$w$ words $\mathcal{B}_1 = (b_{t-1}, \dots, b_{t-t_1})$. 
    Additionally, we compute the checksum using the integer representation of the elements in $\mathcal{B}_1$:
