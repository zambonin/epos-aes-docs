...
\begin{itemize}
    ...
    Seja $X = (x_{t-1}, \dots, x_0) \stackrel{\$}{\longleftarrow} \{0,1\}^m$ be the private key, informally a $t$-tuple of pseudo-random integers. The public key $Y = (y_{t-1}, \dots, y_0)$ can be derived from $X$ by applying $f$ to each of the $x_i$ elements $2^{w}-1$ times. Hence, $Y = (f^{2^{w}-1}(x_{t-1}), \dots, f^{2^{w}-1}(x_0))$.
\end{itemize}
