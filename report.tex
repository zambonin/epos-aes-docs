...
A soma de verificação $c$ é necessária a fim de proteger o esquema contra falsificação de assinaturas. Observe que, em sua ausência, é possível que uma entidade maliciosa crie assinaturas $\sigma'$ apenas calculando mais iterações de $f$ em blocos da assinatura $\sigma$. Com a presença de $c$, e consequentemente $\mathcal{B}_2$, ao modificar algum valor de $\mathcal{B}_1$,
...
\textsc{Key Generation Algorithm $(Kg(1^{n}))$}: 
