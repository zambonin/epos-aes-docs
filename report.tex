...
\begin{enumerate}
    ...
    \item[\textsc{InvSubBytes}] $c = 03 \cdot x^{3} + 01 \cdot x^{2} + 01 \cdot x + 02$, módulo $x^{4} + 1$
    
    \item[\textsc{InvMixColumns}] 
    % $c^{-1} \equiv 01 \pmod{x^4 + 1} \equiv 0B \cdot x^{3} + OD \cdot x^{2} + 09 \cdot x + 0E$
    
    Cada coluna de $A$, $A_j$, é multiplicada pela inversa multiplicativa do polinômio utilizado no \textsc{MixColumns}, módulo $x^{4} + 1$, de valor $c = $.

\end{enumerate}

Assim, o algoritmo resultante pode ser representado pelo pseudocódigo abaixo. Note a mudança da ordem das etapas, e a utilização de $K_e$
