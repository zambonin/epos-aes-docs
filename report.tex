...
A expansão de chaves consiste da criação de um conjunto de palavras $K_e$ de 32 bits, inicialmente baseado na chave $K$. Tome $\ell = \frac{n}{32}$, e assumindo que é necessário criar palavras suficientes para todas as rodadas do algoritmo, então $t = \ell(n_r + 1)$.
Portanto, $K_e = \{k_0, \dots, k_{t - 1}\}$. Inicialmente, $K$ é dividido e as primeiras $\ell$ palavras de
$K_e$ recebem 


$t = \frac{n}{32}(n_r + 1)$, denominado $K_e = \{k_0, \dots, k_{t - 1}\}$. Este tamanho é baseado na divisão de $K$ em palavras de 32 bits e na necessidade de chaves distintas para cada rodada do algoritmo. Então,
$K = \{k_0, \dots, k_{\frac{n}{32} - 1}$, e $\forall i >= $ 
