...
O esquema de assinaturas de unico uso de Winternitz(\textsc{Wots}), em sua proposta original \cite{Merkle:1989:CDS:118209.118230} tem o seguinte funcionamento:
...
\begin{itemize}
    ...
    Pegando a mensagem $M$ e calculando o seu resumo $d = \mathcal{H}(M)$. Por conveniência, nós escolhemos $m$ ou $w$ para que $w \mid m$. Podem ser concatenados $0$ ao resumo para que isso seja satisfeito. $d$ é dividido $t_1$-tupla palavras base-$w$, $\mathcal{B}_1 = (b_{t-1}, \dots, b_{t-t_1})$. 
    Adicionalmente, calculamos o checksum usando a representação inteira dos elementos deusing the integer representation of the elements in $\mathcal{B}_1$:
    $$ c = \sum_{i=t-t_1}^{t-1} 2^w - 1 - b_i.$$
    Again, $c$ may be padded with zeroes until $w \mid \vert c \vert$. Finally, it is split into a     $t_2$-tuple of base-$w$ words $\mathcal{B}_2 = (b_{t_2-1}, \dots, b_0)$. With $\mathcal{B} =     \mathcal{B}_1 \cup \mathcal{B}_2$ computed, we obtain the signature
    $$\alpha = (f^{b_{t-1}}(x_{t-1}), \dots, f^{b_0}(x_0)).$$

    \item\emph{Signature Verification.} To ascertain the correctness of the
