...
\begin{enumerate}
    ...
    \item[] \emph{Geração de chaves.} Defina \sk{} e $r$ como tuplas de inteiros pseudoaleatórios, discutidos anteriormente. O algoritmo também deve escolher uma chave de função $k \stackrel{\$}\longleftarrow \mathcal{K}$. Em seguida, derive \pk{} da seguinte maneira: 
    
    a chave pública de verificação $pk$ é gerada através de $pk = (pk_{0},pk_{1},\dots,pl_{l})=((r,k),c^{w-1}_{k}(sk_{1},r),\dots,c^{w-1}_{k}(sk_{l},r)$
