...
Então, $\forall i \geq \ell, K_{e_{i}} = K_{e_{i-4}} \oplus \mathbf{k}$. A palavra $\mathbf{k}$ é decidida de acordo com as seguintes restrições. Tome $rot(x)$ como o deslocamento circular à esquerda de 8 bits.

\begin{itemize}
    \item $i \equiv 0\pmod{4} \rightarrow \mathbf{k} = \textsc{SubBytes}(rot(K_{e_{i - 1}}))$
\end{itemize}
