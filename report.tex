...
Então, $\forall i \geq \ell, K_{e_{i}} = K_{e_{i - \ell}} \oplus \mathbf{k}$. A palavra $\mathbf{k}$ é decidida de acordo com as restrições abaixo, que podem ser aplicadas ao mesmo tempo. Tome $rot(x)$ como o deslocamento circular à esquerda de 8 bits da palavra $x$, e a lista de constantes $RC$ com elementos em $\mathbb{F}_{2^{8}}$ definida pela recursão $RC_0 = x^0, RC_1 = x^1, RC_j = x^{j-1} \cdot RC_{j-1}, j > 2$.
