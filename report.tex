...
\begin{enumerate}
    \item[] \emph{Geração de chaves.} Defina $\mathcal{S}_k = (x_{t-1}, \dots, x_0) \stackrel{\$}{\longleftarrow} \{0,1\}^m$ como a chave privada, informalmente uma $t$-tupla de inteiros pseudoaleatórios. A chave pública $Y = (y_{t-1}, \dots, y_0)$ pode ser derivada de $\mathcal{S}_k$, computando $\forall x \in \mathcal{S}_k, f^{2^{w}-1}(x)$.
    
    $$ aplicando $f \; 2^{w}-1$ vezes em
    cada elemento $x_i$. Logo, $Y = (f^{2^{w}-1}(x_{t-1}), \dots, f^{2^{w}-1}(x_0))$.
