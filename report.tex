...
\begin{itemize}
    ...
    Pegando a mensagem $M$ e calculando o seu resumo $d = \mathcal{H}(M)$. Por conveniência, nós escolhemos $m$ ou $w$ para que $w \mid m$. Podem ser concatenados zeros ao resumo para que isso seja satisfeito. $d$ é dividido $t_1$-tupla palavras base-$w$, $\mathcal{B}_1 = (b_{t-1}, \dots, b_{t-t_1})$. 
    ...
    Novamente, $c$ pode ser arredondado com zeros até que $w \mid \vert c \vert$. Finalmente, it is split into a     $t_2$-tuple of base-$w$ words $\mathcal{B}_2 = (b_{t_2-1}, \dots, b_0)$. With $\mathcal{B} =     \mathcal{B}_1 \cup \mathcal{B}_2$ computed, we obtain the signature
