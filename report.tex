...
\begin{enumerate}
    \item[] \emph{Geração de chaves.} Defina $X = (x_{t-1}, \dots, x_0) \stackrel{\$}{\longleftarrow} \{0,1\}^m$ como a chave privada, informalmente uma $t$-tupla de inteiros pseudoaleatórios. A chave píubilca $Y = (y_{t-1}, \dots, y_0)$ pode ser derivada de $X$ aplicando-se $f$ para cada $x_i$ elementos $2^{w}-1$ vezes. Logo, $Y = (f^{2^{w}-1}(x_{t-1}), \dots, f^{2^{w}-1}(x_0))$.
