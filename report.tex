...
In Figure \ref{fig:wotssign} we illustrate the signature generation
and its verification. It is visible that computing a signature block
is a step necessarily after the key generation. Hence, the key and
signature generation may only be computed by whoever has the secret
signature key $X$.

\begin{figure}[h!]
    \centering
    \input{charts/wotssign}
    \caption{\textsc{Wots} signature and verification steps
      for a index $i \in \{0, \dots, t-1\}$.}
    \label{fig:wotssign}
\end{figure}
