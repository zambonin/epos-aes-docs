...
\begin{enumerate}
    ...
    \item[] \emph{Verificação da assinatura.} Para se assegurar a corretude da assinatura $\sigma$, cada um de seu blocos $\sigma_i$ precisa ser verificado separadamente através do calculo das aplicações restantes de $f$. A assinatura é verificada corretamente se $$Y = (f^{2^w - 1 - b_{t-1}}(\sigma_{t-1}), \dots, f^{2^w - 1 - b_0}(\sigma_0)).$$
