...
\begin{itemize}
    ...
    Para se assegurar a corretuda da assinatura $\alpha$, cada um de seu blocos $\alpha_i$ precisa ser verificado separadamente através do calculo das aplicações restantes de $f$. The signature verifies correctly if $$Y = (f^{2^w - 1 - b_{t-1}}(\alpha_{t-1}), \dots, f^{2^w - 1 - b_0}(\alpha_0)).$$
