...
Note que não é possível decrementar valores $b_i \in \mathcal{B}$ pois isto implicaria em achar uma pré-imagem de $f$, o que é computacionalmente inviável tratando-se de funções de mão única. Então, resta apenas a tentativa da falsificação de $\sigma$ incrementando algum valor $b_i$. A soma de verificação $c$ é necessária a fim de proteger o esquema contra falsificação de assinaturas. Observe que, em sua ausência, é possível que uma entidade maliciosa crie assinaturas $\sigma'$ apenas calculando mais iterações de $f$ em blocos da assinatura $\sigma$. 
...
\begin{enumerate}
    ...
    \item[] \textsc{Geraão de chaves}: Com a entrada do parâmetro de segurança $n$, em unário, o algoritmo de geração de chaves escolhe uniforme e aleatóramento $l + w - 1$ $n$-bit strings.
