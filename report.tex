...
\begin{algorithm}[H]
    ...
    $A \longleftarrow \textsc{AddRoundKey}(A, \{K^{e}_{0}, \dots, K^{e}_{\ell - 1}\})$\;
...
\begin{enumerate}
    ...
    \item[\textsc{InvShiftRows}] Cada linha de $A$, $A_i$, é deslocada circularmente à direita     $\ell - i$ vezes. 

    \item[\textsc{InvSubBytes}] Realiza-se a substituição de cada elemento da matriz de estado
        pelo seu valor correspondente em uma \emph{substitution-box} (\emph{S-box}), onde os \emph{nibbles} mais e menos significativos, respectivamente, representam a linha e a coluna do elemento na \emph{S-box}.

    \item[\textsc{InvMixColumns}] Cada coluna de $A$, $A_j$, é multiplicada pelo polinômio
        $c = 03 \cdot x^{3} + 01 \cdot x^{2} + 01 \cdot x + 02$, módulo $x^{4} + 1$.
