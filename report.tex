...
Note que não é possível decrementar valores $b_i \in \mathcal{B}$ pois isto implicaria em achar uma pré-imagem de $f$, o que é computacionalmente inviável tratando-se de funções de mão única. EntãoNote que não é possível decrementar valores $b_i \in \mathcal{B}$ pois isto implicaria em achar uma pré-imagem de $f$, o que é computacionalmente inviável tratando-se de funções de mão única. A soma de verificação $c$ é necessária a fim de proteger o esquema contra falsificação de assinaturas. Observe que, em sua ausência, é possível que uma entidade maliciosa crie assinaturas $\sigma'$ apenas calculando mais iterações de $f$ em blocos da assinatura $\sigma$. 


Com a presença de $c$, e consequentemente $\mathcal{B}_2$, ao modificar um valor $b_i$ de $\mathcal{B}_1$, para que o valor correto de $\mathcal{B_2}$ seja mantido,
é necessário que uma pré-imagem
