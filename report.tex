...
Tome $\ell = \frac{n}{32}$, e assumindo que é necessário criar palavras suficientes para utilização em todas as rodadas do algoritmo, então $t = \ell(n_r + 1)$ e $K_e = \{k_0, \dots, k_{t - 1}\}$. Também defina $rot(x)$ como o deslocamento circular à esquerda de 8 bits da palavra $x$, e a lista de constantes $RC$ com elementos em $\mathbb{F}_{2^{8}}$ definida pela recursão $RC_0 = x^0, RC_1 = x^1, RC_j = x^{j-1} \cdot RC_{j-1}, j > 2$. Inicialmente, $\{k_0, \dots, k_{\ell - 1}\} = K$, e $\forall i \geq \ell, K_{e_{i}} = K_{e_{i - \ell}} \oplus \mathbf{k}$. A palavra $\mathbf{k}$ é decidida de acordo com as restrições (mutualmente exclusivas) abaixo.

\begin{itemize}
    \item $i \equiv 0 \pmod{4} \rightarrow \mathbf{k}
        = \textsc{SubBytes}(rot(K_{e_{i - 1}})) \oplus RC_{\frac{i}{\ell}}$;
    \item $\ell = 8 \land i \equiv 4 \pmod{8} \rightarrow \mathbf{k}
        = \textsc{SubBytes}(K_{e_{i - 1}})$.
\end{itemize}
...
\begin{algorithm}[H]
 ...
 $K_{e_0} \dots K_{e_{n_r}} \longleftarrow \textsc{KeyExpansion}(K)$\;
 $A \longleftarrow \textsc{AddRoundKey}(m, \{K_{e_0}, \dots, K_{e_{\ell - 1}}\})$\;
 ...
 \For{$i \leftarrow 0$ \KwTo $n_r - 2$}{
   ...
   $A \longleftarrow \textsc{AddRoundKey}(A,
    \{K_{e_{\ell \times i}}, \dots, K_{e_{\ell \times (i + 1)}}\})$\;
 }
 
 $A \longleftarrow \textsc{SubBytes}(A)$\;
 $A \longleftarrow \textsc{ShiftRows}(A)$\;
 $A \longleftarrow \textsc{AddRoundKey}(A, \{K_{e}\})$\;
\end{algorithm}


$C = E(m, K)$. Para que a cifra seja caracterizada como simétrica, é preciso criar uma função $m = D(C, K)$. Isto pode ser obtido da seguinte maneira: 

este processo, $K_e$
% explicar como descriptografa
