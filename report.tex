...
\begin{itemize}
    \item Assumimos um parâmetro de segurança $m$, geralmente com o tamanho da saida de uma função de resumo cryptografico e escolhemos um parâmetro $w \in \mathbb{N}, w > 1$. 
    \item Considerando a função *(non-compressing one-way)* $f : \{0, 1\}^{m} \longrightarrow \{0, 1\}^{m}$ e a função de resumo
