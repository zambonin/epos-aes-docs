...
A soma de verificação é necessária a fim de proteger o esquema contra 

\subsection{\textsc{Wots+}}

O principal esquema variante do \textsc{Wots}, chamado de \textsc{Wots+}~\cite{cryptoeprint:2017:965}, tem como ideia principal a substituição da função de mão única $f$ por uma família $\mathcal{F}_k$ com entradas pseudoaleatórias, substituindo o processo de iterações sequenciais --- utilizando apenas uma função --- do esquema original. Esta característica também habilita uma prova de segurança mais simples, mas não menos poderosa, desconsiderando a necessidade da resistência à colisões de $f$.

Diferente de outras variantes, onde essas cadeias são apenas iterações sequênciais da mesma função, o Wots+ usa um modo especial de iterações a qual habilita uma prova de segurança *tight*, sem que a família da função de resumo usada sejam a prova de colisão.
A família de funções que o Wots+ utiliza é definida por 
$\mathcal{F}_k :\{f_k:\{0,1\}^{n}\rightarrow \{0,1\}^{n}\mid k\in \mathcal{K}_n\}$ com um espaço de chaves $\mathcal{K}_n$. Pode
