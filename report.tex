...
O AES é uma cifra de blocos que opera sobre uma matriz de estado de $4 \times 4$ bytes. Estas operações são caracterizadas por adições, multiplicações e divisões sobre um corpo finito
$\mathbb{F}_{2^{8}}$ definido pelo polinômio irredutível $m(x) = x^{8} + x^{4} + x^{3} + x + 1$
\footnote{Operações nesta estrutura algébrica são similares a...}. Seu funcionamento é denominado
iterativo, ou seja, o estado é modificado através de várias subetapas.
