...
\begin{itemize}
    ...
    Para se assegurar a corretuda da assinatura $\alpha$, each of its blocks $\alpha_i$ need to be verified separately by computing the remaining applications of $f$. The signature verifies correctly if $$Y = (f^{2^w - 1 - b_{t-1}}(\alpha_{t-1}), \dots, f^{2^w - 1 - b_0}(\alpha_0)).$$
