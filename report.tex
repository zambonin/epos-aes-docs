...
\begin{itemize}
    ...
    Take a message $M$ and compute its digest $d = \mathcal{H}(M)$. For convenience, we choose $m$ or $w$ such that $w \mid m$. However, one may pad the digest by appending zeros to satisfy this requirement. $d$ is then split into a $t_1$-tuple of base-$w$ words $\mathcal{B}_1 = (b_{t-1}, \dots, b_{t-t_1})$. 
