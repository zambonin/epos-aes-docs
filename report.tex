...
representada pelo pseudocódigo abaixo. Note que \textsc{KeyExpansion} retorna palavras organizadas em conjuntos de tamanho suficiente para que sejam operadas com $A$.

\begin{algorithm}[H]
    \KwData{$m$, o texto a ser cifrado; $K$, a chave desejada}
    $K_{e_0} \dots K_{e_{n_r}} \longleftarrow \textsc{KeyExpansion}(K)$\;
    $A \longleftarrow \textsc{AddRoundKey}(m, K^{e}_{0})$\;
    ...
    \For{$i \leftarrow 1$ \KwTo $n_r - 1$}{
        ...
        $A \longleftarrow \textsc{AddRoundKey}(A, K^{e}_{i})$\;
    }
 
    $A \longleftarrow \textsc{SubBytes}(A)$\;
    $A \longleftarrow \textsc{ShiftRows}(A)$\;
    $A \longleftarrow \textsc{AddRoundKey}(A, K^{e}_{n_r})$\;

    $c \longleftarrow A$\;
    \KwResult{$c$, o texto cifrado resultante}
\end{algorithm}

Para que a cifra seja caracterizada como simétrica, é preciso criar uma função $m = D(C, K)$. Isto pode ser obtido da seguinte maneira: 
